...
def line_intersection(p1, p2, q1, q2):
    """Find the intersection point of two line segments (p1p2 and q1q2)."""
    def ccw(A, B, C):
    ...
    if ccw(A, C, D) != ccw(B, C, D) and ccw(A, B, C) != ccw(A, B, D):
        ...
        intersect_y = ((A[0] * B[1] - A[1] * B[0]) * (D[1] - C[1]) - (A[1] - B[1]) * (D[0] * C[0] - D[1] * C[0])) / denominator
        intersect_point = (intersect_x, intersect_y)
        if is_between(A, intersect_point, B) and is_between(C, intersect_point, D):
...
def sur(angle):
    ...
    global points
    global point_pairs
    return a2n(angle[0]) in surrounding_angle(a2n(angle[1])) and\
           a2n(angle[2]) in surrounding_angle(a2n(angle[1]))

def print_angle(a, b, c):
    global lines
    global matrix
    global matrix_eq
    global points
...
def print_angle_2(angle):
    ...
    global point_pairs
    x = angle
    return print_angle(a2n(x[0]), a2n(x[1]), a2n(x[2]))
...
def process():
    ...
    for i in range(len(points)):
        for angle in itertools.combinations(surrounding_angle(i), 2):
            if angle[0] != angle[1]:
                output.append(print_angle(angle[0], i, angle[1]))
...
def all_cycle(graph):
    ...
    path = []
    for cy in cycles:
        path.append([node for node in cy])
    return path
...
def all_triangle(all_angles):
    ...
    for x in new_cycle:
        ...
        for i in range(-2, len(x)-2, 1):
            ...
            out.append("".join([n2a(y) for y in angle]))
        print(" + ".join(out) + " = " + str(180*(len(x)-2)))
        for i in range(len(out)):
            if a2n(out[i][0]) > a2n(out[i][-1]):
                out[i] = out[i][-1] + out[i][1] + out[i][0]
            if sur(out[i]):
                out[i]= print_angle_2(out[i])
        matrix.append([0]*len(all_angles))
...
def two_times(angle_name):
    angle_name = print_angle_2(angle_name)
    return print_angle_2(angle_name)
def angle_eq(angle_name, val):
    ...
    if sur(angle_name):
        ...
        angle_name = two_times(angle_name)
